...
public class JsonVectorStore(string basePath) : IVectorStore
{
        ...
        where TKey : notnull
    {
        ...
        {
            if (_records is null) await CreateCollectionAsync(cancellationToken);
        }
            ...
            [EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            ...
            foreach (var key in results) yield return key;
        }

        public Task<VectorSearchResults<TRecord>> VectorizedSearchAsync<TVector>(TVector vector,
            VectorSearchOptions<TRecord>? options = null, CancellationToken cancellationToken = default)
        {
            ...
            if (options?.Filter is { } filter) filteredRecords = filteredRecords.AsQueryable().Where(filter);
